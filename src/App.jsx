...
export default function App() {
  ...
  const [menuItemData, setMenuItemData] = useState({ category: 'kahvalti', name: '', price: '', description: '', image: '', isFeatured: false });
  ...
  useEffect(() => {
    ...
    return () => { restoranUnsub(); matchUnsub(); reqUnsub(); menuUnsub(); };
  }, [user]);
  ...
  const submitRequest = async (e) => {
    ...
    try {
      ...
      setTimeout(() => { setShowRequestModal(false); setRequestSuccess(false); setRequestData({ type: 'yemek', name: '', phone: '', peopleCount: 2, date: getToday(), notes: '' }); setRequestError(''); }, 3000);
  ...
  const handleFormSubmit = async (e, isMac) => {
    ...
    const initialForm = { type: isMac ? 'mac' : 'yemek', name: '', phone: '', notes: '', peopleCount: 1, table: '', date: getToday() };
    ...
    try {
      ...
      setForm({ ...initialForm, date: cleanData.date }); setShowTableMap(false); setFunc('');
  ...
  const handleMenuSubmit = async (e) => {
    e.preventDefault();
    if (!user) return;
    if (!menuItemData.name?.trim()) { setMenuErrorMsg("Ürün adı zorunludur."); return; }
    try {
      if (isMenuEditing) {
        await updateDoc(doc(db, 'menuItems', isMenuEditing), { ...menuItemData, updatedAt: new Date().toISOString() });
        setIsMenuEditing(null);
      } else {
        await addDoc(collection(db, 'menuItems'), { ...menuItemData, createdAt: new Date().toISOString() });
      }
      setMenuItemData({ category: 'kahvalti', name: '', price: '', description: '', image: '', isFeatured: false }); setMenuErrorMsg('');
    } catch (err) { setMenuErrorMsg("Kayıt hatası."); }
  };
      ...
      try {
        for (const cat of DEFAULT_MENU_ITEMS) {
          for (const item of cat.items) {
            await addDoc(collection(db, 'menuItems'), { category: cat.cat, name: item.n, price: '', description: '', image: item.i || '', isFeatured: item.f || false, tag: item.t || '', createdAt: new Date().toISOString() });
          }
        }
        alert("Menü başarıyla aktarıldı!");
      } catch (err) { alert("Aktarım sırasında hata oluştu."); }
    }
  };
  ...
  const executeDelete = async (id, colName) => {
    ...
    try {
      ...
      if (colName === 'reservations') { setDeleteConfirmId(null); setIsEditing(null); } 
      else if (colName === 'matchReservations') { setMatchDeleteConfirmId(null); setIsMatchEditing(null); } 
      else if (colName === 'menuItems') { setMenuDeleteConfirmId(null); setIsMenuEditing(null); }
    } catch (err) {}
  };
  ...
  const getHistoryData = () => {
    let allData = [];
    if (historyTypeFilter === 'all' || historyTypeFilter === 'restoran') allData = [...allData, ...reservations.map(r => ({ ...r, eventType: typeLabels[r.type] || 'Genel' }))];
    if (historyTypeFilter === 'all' || historyTypeFilter === 'mac') allData = [...allData, ...matchReservations.map(r => ({ ...r, eventType: 'Maç' }))];
    ...
    <div className={`fixed top-0 left-0 w-full z-50 flex justify-center pointer-events-none`}>
      <nav className={`w-full pointer-events-auto transition-all duration-500 ${isScrolled || isDark ? 'bg-black/90 backdrop-blur-md shadow-md py-3 border-b border-white/10' : 'bg-transparent py-5 sm:py-6'}`}>
        <div className="w-full px-4 sm:px-8 lg:px-12 xl:px-24 flex items-center justify-between">
          <div className={`transition-all duration-500 cursor-pointer flex items-center justify-center shrink-0 ${isScrolled || isDark ? 'h-10 sm:h-12' : 'h-12 sm:h-16'}`} onClick={() => setCurrentView('landing')}>
          ...
          <div className="shrink-0 flex items-center justify-end gap-3 sm:gap-4">
            <button onClick={() => setShowRequestModal(true)} className={`hidden sm:flex items-center gap-2 px-4 py-2.5 xl:px-6 xl:py-3 rounded-full text-xs xl:text-sm font-bold tracking-widest uppercase transition-all shadow-md ${isScrolled && !isDark ? 'bg-white text-emerald-700 border border-emerald-200 hover:bg-emerald-50' : 'bg-emerald-700/80 backdrop-blur-sm text-white hover:bg-emerald-600 border border-emerald-500/50'}`}>
              <CalendarDays size={16} /> Rezervasyon
            </button>
            <button onClick={() => {setCurrentView('menu'); window.scrollTo(0,0);}} className="shine-effect bg-black/80 text-[#FBE18D] border border-[#FBE18D]/30 px-5 py-2.5 sm:px-6 sm:py-3 xl:px-8 xl:py-3.5 rounded-full text-xs sm:text-sm font-black tracking-widest uppercase hover:bg-black hover:border-[#FBE18D] hover:scale-105 transition-all shadow-lg whitespace-nowrap flex items-center gap-2">
  ...
  const renderFooter = () => (
    <footer id="iletisim" className="w-full bg-slate-950 text-slate-400 py-20 lg:py-24 relative z-10 border-t border-slate-800">
      <div className="w-full mx-auto px-6 sm:px-12 lg:px-24 grid grid-cols-1 md:grid-cols-3 gap-16 lg:gap-24">
        ...
        <div className="flex flex-col items-center md:items-start text-center md:text-left">
          ...
          <ul className="space-y-6 text-base lg:text-lg font-medium text-slate-300">
            <li><div className="flex flex-col items-center md:items-start gap-2"><div className="flex items-center justify-center md:justify-start gap-3"><MapPin size={20} className="text-orange-500 shrink-0"/> <span className="text-white font-bold">📍 Harita</span></div><p className="ml-0 md:ml-8 text-slate-400">Gebze, Kocaeli</p><a href="https://www.google.com/maps/dir/?api=1&destination=Salaaş+Cafe+Restaurant+Gebze" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-colors shadow-md mt-1 ml-0 md:ml-8 border border-slate-700 w-max">Yol Tarifi Al <ArrowRight size={16}/></a></div></li>
            <li><a href={`tel:+${WHATSAPP_NO}`} className="hover:text-orange-400 transition-colors flex items-center justify-center md:justify-start gap-3"><Phone size={20} className="text-orange-500 shrink-0"/> <span className="text-white font-bold">📞 Telefon</span><span className="ml-2">0536 017 02 08</span></a></li>
        ...
        <div className="flex flex-col items-center md:items-start text-center md:text-left">
          ...
          <ul className="space-y-5 text-base lg:text-lg font-medium mb-10 text-slate-300">
            <li><button onClick={() => {setCurrentView('menu'); window.scrollTo(0,0);}} className="hover:text-white transition-colors flex items-center justify-center md:justify-start gap-3"><ChevronRight size={18} className="text-orange-500"/> Dijital Menü</button></li>
          </ul>
      ...
      <div className="w-full mx-auto px-6 sm:px-12 lg:px-24 mt-20 pt-10 border-t border-slate-800/50 text-center text-sm font-medium opacity-60"><p>© 2026 Salaaş Cafe Restaurant. Tüm hakları saklıdır.</p></div>
    </footer>
  );
  ...
  if (currentView === 'menu') {
     return (
        <div className="min-h-screen bg-[#0a0a0a] font-sans text-slate-200 relative w-full overflow-x-hidden">
                   ...
                   return (
                     <div id={`cat-${cat.id}`} key={cat.id} className="scroll-mt-36">
                        <div className="flex items-center gap-4 mb-8">
                           <div className="bg-gradient-to-br from-orange-500 to-yellow-600 w-12 h-12 rounded-full flex items-center justify-center text-white shadow-[0_0_15px_rgba(249,115,22,0.3)] shrink-0"><CatIcon size={20} /></div>
          ...
          <footer className="w-full bg-black py-10 border-t border-white/10 text-center relative z-20 mt-12"><p className="text-slate-500 font-medium text-sm">© 2026 Salaaş Cafe Restaurant. Tüm hakları saklıdır.</p></footer>
        </div>
     );
  }
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 w-full col-span-12">
                <div className="lg:col-span-4 xl:col-span-3 space-y-6 w-full">
                  ...
                  <div className={`bg-white rounded-3xl shadow-xl border w-full ${isMenuEditing ? 'border-orange-400' : 'border-slate-200'}`}>
                    <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center"><h2 className="font-black text-lg">{isMenuEditing ? 'Ürünü Düzenle' : 'Yeni Ürün Ekle'}</h2>{isMenuEditing && <button onClick={()=>{setIsMenuEditing(null); setMenuItemData({category: 'kahvalti', name: '', price: '', description: '', image: '', isFeatured: false});}}><X size={18}/></button>}</div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 relative flex flex-col scroll-smooth w-full overflow-x-hidden">
      ...
      <header className="relative w-full min-h-[500px] h-[85vh] bg-slate-900 flex items-center justify-center overflow-hidden pt-16">
         <div className="absolute inset-0 z-0"><img src="/salaasarkaplan.jpeg" alt="Arka Plan" className="w-full h-full object-cover opacity-50" /><div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/60 to-transparent"></div></div>
         <div className="relative z-10 text-center px-4 w-full flex flex-col items-center">
            <h1 className="text-5xl sm:text-7xl font-black text-white font-serif mb-6 drop-shadow-2xl">Lezzet ve <span className="text-orange-400">Muhabbetin</span> Adresi</h1>
            <p className="text-lg sm:text-2xl text-slate-200 max-w-3xl font-medium mb-10 drop-shadow-lg">Şehrin gürültüsünden uzak, samimi atmosferimizde unutulmaz anılar biriktirin.</p>
            <button onClick={() => setShowRequestModal(true)} className="bg-emerald-600 hover:bg-emerald-500 text-white px-10 py-5 rounded-full font-black uppercase tracking-widest shadow-2xl flex items-center gap-2 mb-10"><CalendarDays size={24} /> Rezervasyon Talebi</button>
         </div>
      </header>

      <main className="w-full relative z-10 flex-1">
        <section id="hakkimizda" className="w-full max-w-5xl mx-auto px-4 py-20 text-center">
          <MoonStar size={56} className="text-orange-400 opacity-80 mx-auto mb-6" />
          <h2 className="text-sm font-black tracking-[0.3em] text-orange-500 uppercase mb-4">Hikayemiz</h2>
          <h3 className="text-4xl sm:text-5xl font-serif font-black text-[#0B3B2C] mb-8">Sıcak, Samimi ve Lezzetli</h3>
          <p className="text-lg sm:text-xl text-slate-600 leading-relaxed">Salaaş Cafe Restaurant olarak, misafirlerimize kendilerini evlerinde hissedecekleri sıcak bir ortam sunuyoruz. Özenle seçilmiş malzemelerle hazırladığımız zengin menümüzle günün her saatinde kaliteli bir deneyim yaşatmayı hedefliyoruz.</p>
        </section>

        <section id="lezzetler" className="w-full py-20 bg-slate-50">
          <div className="max-w-7xl mx-auto px-4">
            <div className="flex justify-between items-end mb-12">
              <div><h2 className="text-sm font-black tracking-[0.3em] text-orange-500 uppercase mb-2">Vitrinimiz</h2><h3 className="text-4xl font-serif font-black text-[#0B3B2C]">Bunu Denediniz mi?</h3></div>
              <button onClick={() => navigateTo('menu')} className="flex items-center gap-2 font-bold text-slate-500 hover:text-orange-600 uppercase tracking-widest">Tüm Menüyü Gör <ChevronRight size={18}/></button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {activeGallery.slice(0, 4).map((item) => (
                <div key={item.id || item.name} onClick={() => navigateTo('menu')} className="lg:col-span-2 rounded-3xl overflow-hidden shadow-lg h-80 sm:h-[400px] relative cursor-pointer group">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-900/90 to-transparent flex flex-col justify-end p-8"><span className="bg-orange-500 text-white text-xs font-black uppercase tracking-widest px-4 py-1.5 rounded-full w-max mb-4 flex items-center gap-1.5"><Star size={14}/> {item.tag || 'Öne Çıkan'}</span><h4 className="text-white font-serif font-black text-3xl">{item.name}</h4></div>
                </div>
              ))}
            </div>
          </div>
      ...
      {showRequestModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col">
            <div className="bg-emerald-700 p-6 flex justify-between text-white"><h3 className="font-black text-xl flex items-center gap-2"><CalendarDays/> Rezervasyon Talebi</h3><button onClick={() => setShowRequestModal(false)}><X/></button></div>
            <div className="p-8">
              {requestSuccess ? (
                 <div className="text-center py-10"><CheckCircle2 size={64} className="mx-auto text-emerald-500 mb-4"/><h3 className="text-2xl font-black text-slate-800">Talebiniz Alındı!</h3><p className="text-slate-500 mt-2">En kısa sürede onay için size dönüş yapılacaktır.</p></div>
              ) : (
                <form onSubmit={submitRequest} className="space-y-4">
                  {requestError && <div className="text-red-500 font-bold text-sm bg-red-50 p-3 rounded-lg">{requestError}</div>}
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Rezervasyon Türü</label>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                        {['kahvalti', 'yemek', 'mac', 'organizasyon'].map(t => (
                          <button key={t} type="button" onClick={() => setRequestData({...requestData, type: t})} className={`py-3 rounded-xl font-bold text-sm border-2 ${requestData.type === t ? 'border-orange-500 bg-orange-50 text-orange-700' : 'border-slate-200 text-slate-500'}`}>{typeLabels[t]}</button>
                        ))}
                    </div>
                  </div>
                  <div className="flex gap-4">
                    <div className="flex-[2]"><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Ad Soyad</label><input type="text" name="name" value={requestData.name} onChange={handleRequestChange} className="w-full p-3 rounded-xl border-2 border-slate-200 font-bold" required /></div>
                    <div className="flex-[1]"><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Kişi</label><input type="number" name="peopleCount" min="1" value={requestData.peopleCount} onChange={handleRequestChange} className="w-full p-3 rounded-xl border-2 border-slate-200 font-bold text-center" required /></div>
                  </div>
                  <div className="flex gap-4">
                    <div className="flex-1"><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Telefon</label><input type="tel" name="phone" value={requestData.phone} onChange={handleRequestChange} className="w-full p-3 rounded-xl border-2 border-slate-200 font-bold" required /></div>
                    <div className="flex-1"><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Tarih</label><input type="date" name="date" value={requestData.date} onChange={handleRequestChange} className="w-full p-3 rounded-xl border-2 border-slate-200 font-bold" required /></div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Detaylı Açıklama</label>
                    <textarea name="notes" value={requestData.notes} onChange={handleRequestChange} rows="2" className="w-full p-3 rounded-xl border-2 border-slate-200 font-medium resize-none" required={requestData.type === 'organizasyon'}></textarea>
                  </div>
                  <button type="submit" className="w-full bg-emerald-600 text-white font-black py-4 rounded-xl mt-2">GÖNDER</button>
                </form>
              )}
            </div>
          </div>
      ...
      {showLoginModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="bg-[#0B3B2C] p-6 flex justify-between text-white"><h3 className="font-black text-lg"><Lock className="inline mr-2"/> Sistem Girişi</h3><button onClick={() => setShowLoginModal(false)}><X/></button></div>
            <form onSubmit={handleLogin} className="p-6 space-y-4">
              {loginError && <div className="text-red-500 font-bold text-sm bg-red-50 p-2 rounded">{loginError}</div>}
              <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Kullanıcı Adı</label><input type="text" value={loginUser} onChange={(e) => setLoginUser(e.target.value)} className="w-full p-3 rounded-xl border-2 border-slate-200 font-bold" autoFocus /></div>
              <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Şifre</label><input type="password" value={loginPass} onChange={(e) => setLoginPass(e.target.value)} className="w-full p-3 rounded-xl border-2 border-slate-200 font-bold" /></div>
              <button type="submit" className="w-full bg-[#0B3B2C] text-white font-black py-4 rounded-xl mt-2">GİRİŞ YAP</button>
            </form>
          </div>
        </div>
      )}
      
      <a href={`https://wa.me/${WHATSAPP_NO}?text=Merhaba%20Salaas%20Cafe,%20rezervasyon%20yapmak%20istiyorum.`} target="_blank" rel="noreferrer" className="fixed bottom-6 right-6 z-50 bg-[#25D366] text-white p-4 rounded-full font-black shadow-2xl flex items-center gap-2 hover:scale-110 transition-transform">
        <span className="text-2xl">💬</span><span className="hidden sm:block text-sm uppercase">WhatsApp</span>
      </a>
    </div>
  );
}
