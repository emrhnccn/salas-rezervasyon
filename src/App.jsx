...
const MENU_GALLERY = [
  ...
  { id: '3', name: 'Kahvaltı Tabağı', image: '/kahvaltitabagi.jpg' },
  { id: '4', name: 'Köz Patlıcanlı Tavuk', image: '/közpatlıcanlıtavukbnfile.jpg', tag: 'Şefin Tavsiyesi' },
  { id: '5', name: 'Ispanak Yatağında Tavuk', image: '/ıspanakyatagındatavuk.jpg' },
  { id: '6', name: 'Etli Bowl Tabağı', image: '/etlibowltabagi.jpg' },
  { id: '7', name: 'Üç Renkli Tortellini', image: '/ucrenklitortellini.jpg' },
  { id: '8', name: 'Cheddar Çıtır Makarna', image: '/chedarcitirmakarna.jpg' },
  { id: '9', name: 'Cool Lime', image: '/coollime.jpg', tag: 'Ferahlatıcı' },
  { id: '10', name: 'Ice Mocha', image: '/icemocha.jpg' },
...
export default function App() {
  ...
  const [matchFormData, setMatchFormData] = useState({ ...initialFormState, type: 'mac' });
  ...
  useEffect(() => {
    ...
    if (window.location.pathname === '/menu') setShowMenuPage(true);
  ...
  useEffect(() => {
    ...
    return () => { restoranUnsub(); matchUnsub(); reqUnsub(); };
  }, [user]);
  ...
  const handleRequestChange = (e) => {
    ...
    setRequestError('');
    setRequestData(prev => ({ ...prev, [name]: name.includes('Count') ? (value === '' ? '' : parseInt(value, 10)) : value }));
  };
  ...
  const submitRequest = async (e) => {
    ...
    if ((requestData.type === 'dogum_gunu' || requestData.type === 'organizasyon') && (!requestData.notes || !requestData.notes.trim())) {
      setRequestError("Lütfen detaylı açıklama giriniz."); return;
    }
    try {
      const submissionData = { ...requestData, phone: requestData.phone.trim(), peopleCount: parseInt(requestData.peopleCount, 10) || 1, status: 'pending', createdAt: new Date().toISOString(), createdBy: user.uid };
  ...
  const handleFormSubmit = async (e, isMac) => {
    ...
    if (!currentForm.name?.trim()) { setFunc("İsim alanını doldurun."); return; }
    const cleanData = { ...currentForm, phone: currentForm.phone?.trim() || '', notes: currentForm.notes?.trim() || '', peopleCount: parseInt(currentForm.peopleCount, 10) || 1 };
    
    try {
      if (currentEdit) { 
        await updateDoc(doc(db, colName, currentEdit), { ...cleanData, updatedAt: new Date().toISOString() }); 
        isMac ? setIsMatchEditing(null) : setIsEditing(null); 
      } else { 
        await addDoc(collection(db, colName), { ...cleanData, createdAt: new Date().toISOString(), createdBy: user.uid, isArrived: false }); 
      }
      setForm({ ...initialForm, date: cleanData.date }); 
      setShowTableMap(false);
    } catch (err) { setFunc("Kayıt hatası."); }
  };

  const handleEditClick = (res, isMac) => { 
    if(isMac) { setMatchFormData({ ...res, phone: res.phone || '', notes: res.notes || '' }); setIsMatchEditing(res.id); setSelectedMatchDate(res.date); }
    else { setFormData({ ...res, phone: res.phone || '', notes: res.notes || '' }); setIsEditing(res.id); setSelectedFilterDate(res.date); }
    window.scrollTo({ top: 0, behavior: 'smooth' }); 
  };
  
  const cancelEdit = (isMac) => { 
    if(isMac) { setIsMatchEditing(null); setMatchFormData({ ...initialFormState, type: 'mac', date: selectedMatchDate }); setMatchErrorMsg(''); }
    else { setIsEditing(null); setFormData({ ...initialFormState, date: selectedFilterDate }); setErrorMsg(''); }
  };
  ...
  const executeDelete = async (id, colName) => {
    ...
    try {
      ...
      if (colName === 'reservations') { setDeleteConfirmId(null); if (isEditing === id) cancelEdit(false); } 
      else { setMatchDeleteConfirmId(null); if (isMatchEditing === id) cancelEdit(true); }
    } catch (err) {}
  };
  ...
  const sendBulkWhatsApp = (reservationsList, type) => {
    if (!reservationsList || reservationsList.length === 0) { 
      setBulkMessage("Gönderilecek kayıt yok."); setTimeout(() => setBulkMessage(''), 5000); return; 
    }
    const validReservations = reservationsList.filter(res => res.phone && res.phone.trim().length >= 10);
    if (validReservations.length === 0) { 
      setBulkMessage("Geçerli telefon numarası yok."); setTimeout(() => setBulkMessage(''), 5000); return; 
    }
        ...
        <div className={`bg-white/95 backdrop-blur-sm rounded-3xl shadow-xl border overflow-hidden transition-colors w-full ${cEdit ? (isMac ? 'border-blue-400 shadow-blue-500/20' : 'border-orange-400 shadow-orange-500/20') : 'border-slate-200/60'}`}>
          ...
          <form onSubmit={(e) => handleFormSubmit(e, isMac)} className="p-6 lg:p-8 space-y-5">
            ...
            <div className="flex flex-col gap-4">
              <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">İsim</label><input type="text" name="name" value={cForm.name} onChange={(e) => isMac ? handleMatchChange(e) : handleChange(e)} className={`w-full px-5 py-3.5 rounded-xl border-2 border-slate-200 focus:ring-4 ${isMac ? 'focus:ring-blue-500/10 focus:border-blue-500' : 'focus:ring-orange-500/10 focus:border-orange-500'} bg-slate-50/50 font-bold text-lg`} placeholder="Müşteri İsmi" /></div>
              <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Telefon</label><div className="relative"><Phone className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="tel" name="phone" value={cForm.phone} onChange={(e) => isMac ? handleMatchChange(e) : handleChange(e)} className={`w-full pl-11 pr-5 py-3.5 rounded-xl border-2 border-slate-200 focus:ring-4 ${isMac ? 'focus:ring-blue-500/10 focus:border-blue-500' : 'focus:ring-orange-500/10 focus:border-orange-500'} bg-slate-50/50 font-bold text-lg`} placeholder="05XX..." /></div></div>
            </div>
            ...
            <div className={`bg-slate-50 rounded-2xl p-5 border border-slate-200 shadow-inner`}>
              ...
              <div className="flex gap-4">
                <div className="flex-[2]"><div className="relative"><Armchair className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="text" name="table" value={cForm.table} onChange={(e) => isMac ? handleMatchChange(e) : handleChange(e)} className={`w-full pl-11 pr-5 py-3.5 rounded-xl border-2 border-slate-200 focus:ring-4 ${isMac ? 'focus:ring-blue-500/10 text-[#0a192f]' : 'focus:ring-orange-500/10 text-[#0B3B2C]'} bg-white font-black uppercase shadow-sm text-lg`} placeholder="Masa Kodu" /></div></div>
                <div className="flex-[1]"><div className="relative"><Users className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="number" inputMode="numeric" name="peopleCount" min="1" value={cForm.peopleCount} onChange={(e) => isMac ? handleMatchChange(e) : handleChange(e)} className={`w-full pl-11 pr-5 py-3.5 rounded-xl border-2 border-slate-200 focus:ring-4 ${isMac ? 'focus:ring-blue-500/10 text-[#0a192f]' : 'focus:ring-orange-500/10 text-[#0B3B2C]'} bg-white font-black shadow-sm text-lg`} /></div></div>
              </div>
            </div>
            
            <div className="flex flex-col gap-4">
              <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1"><CalendarDays size={14} className={isMac ? "text-blue-500" : "text-orange-500"}/> Tarih</label><input type="date" name="date" value={cForm.date} onChange={(e) => isMac ? handleMatchChange(e) : handleChange(e)} className={`w-full px-5 py-3.5 rounded-xl border-2 border-slate-200 focus:ring-4 ${isMac ? 'focus:ring-blue-500/10 text-[#0a192f]' : 'focus:ring-orange-500/10 text-[#0B3B2C]'} bg-slate-50/50 font-bold text-lg`} required /></div>
              <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1"><MessageSquareText size={14}/> Not / Açıklama</label><textarea name="notes" value={cForm.notes} onChange={(e) => isMac ? handleMatchChange(e) : handleChange(e)} rows={2} className={`w-full px-5 py-3.5 rounded-xl border-2 border-slate-200 focus:ring-4 ${isMac ? 'focus:ring-blue-500/10' : 'focus:ring-amber-500/10'} bg-amber-50/50 font-medium text-lg resize-none`} placeholder="Özel istekler..."></textarea></div>
            </div>
            <button type="submit" className={`w-full font-black tracking-widest uppercase py-5 mt-6 rounded-2xl transition-all shadow-xl flex items-center justify-center gap-3 text-white hover:scale-[1.02] active:scale-95 text-lg ${cEdit ? (isMac ? 'bg-gradient-to-r from-[#0a192f] to-blue-900' : 'bg-gradient-to-r from-[#0B3B2C] to-emerald-900') : (isMac ? 'bg-gradient-to-r from-blue-500 to-cyan-600' : 'bg-gradient-to-r from-orange-500 to-orange-600')}`}>{cEdit ? <Check size={24} /> : <Plus size={24} />} {cEdit ? 'Kaydet' : 'Ekle'}</button>
          </form>
        </div>
      </div>
    );
        ...
        <div className={`bg-gradient-to-br ${isMac ? 'from-[#0a192f] to-blue-900' : 'from-[#0B3B2C] to-emerald-900'} rounded-3xl p-6 lg:p-8 shadow-xl flex flex-col gap-6 relative overflow-hidden w-full ${printSingleId ? 'print:hidden' : 'print:bg-white print:from-white print:to-white print:border-b-2 print:border-black print:rounded-none print:shadow-none print:p-2 print:mb-4'}`}>
          <div className={`flex flex-col sm:flex-row items-start sm:items-center justify-between w-full ${isMac ? 'border-blue-800/50' : 'border-emerald-700/50'} border-b pb-6 z-10 print:border-black print:pb-2 gap-4`}>
            <div className={`flex items-center gap-5 ${isMac ? 'text-cyan-200' : 'text-[#FBE18D]'} w-full sm:w-auto`}>
              ...
              <div><p className={`text-sm font-black uppercase tracking-widest ${isMac ? 'text-cyan-400' : 'text-orange-400'} print:text-black`}>{title}</p><p className="text-3xl sm:text-4xl font-black text-white mt-1 print:text-black">Toplam: <span className={`${isMac ? 'text-cyan-400' : 'text-orange-400'} print:text-black`}>{tPeople}</span> Kişi</p></div>
            </div>
        ...
        <div className="bg-white/95 backdrop-blur-sm rounded-3xl shadow-sm border border-slate-200/60 p-6 sm:p-8 lg:p-10 min-h-[500px] print:p-0 print:border-none print:shadow-none print:bg-white w-full">
          <div className={`flex flex-col md:flex-row items-start md:items-center justify-between mb-10 pb-6 border-b border-slate-100 gap-5 ${printSingleId ? 'print:hidden' : 'print:border-b-2 print:border-black print:pb-2 print:mb-3'}`}>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
              <div className="relative w-full sm:w-80 print:hidden"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={20} /><input type="text" placeholder="İsim, masa veya telefon ara..." value={cSearch} onChange={(e) => setCSearch(e.target.value)} className={`w-full pl-12 pr-5 py-3 rounded-xl border-2 border-slate-200 focus:ring-4 ${isMac ? 'focus:ring-blue-500/10 focus:border-blue-500' : 'focus:ring-orange-500/10 focus:border-orange-500'} text-base font-medium shadow-sm outline-none bg-slate-50 transition-all`} /></div>
              <span className={`${isMac ? 'bg-blue-100 text-blue-800' : 'bg-orange-100 text-orange-800'} px-5 py-3 rounded-full text-sm font-black print:hidden shrink-0`}>{cList.length} Kayıt</span>
              <button onClick={() => window.print()} className={`w-full sm:w-auto ${isMac ? 'bg-[#0a192f] hover:bg-blue-900' : 'bg-slate-800 hover:bg-slate-700'} text-white px-6 py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-colors print:hidden shrink-0`}><Printer size={18} /> YAZDIR</button>
            </div>
          </div>
          
          {cList.length === 0 ? (
            <div className="bg-slate-50/50 rounded-3xl border-2 border-dashed border-slate-200 p-24 text-center text-slate-400 print:hidden w-full"><Search size={56} className={`opacity-50 ${isMac ? 'text-blue-400' : 'text-orange-400'} mx-auto mb-5`} /><p className="font-black text-2xl">Kayıt bulunamadı.</p></div>
          ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6 lg:gap-8 w-full">
                   {pendingRequests.map((req) => (
                     <div key={req.id} className="bg-slate-50 border-2 border-slate-200 rounded-3xl p-6 lg:p-8 hover:shadow-xl transition-all relative overflow-hidden group w-full flex flex-col">
                       <div className={`absolute top-0 right-0 px-6 py-2 rounded-bl-2xl font-black text-xs uppercase tracking-widest text-white shadow-md ${req.type === 'mac' ? 'bg-blue-600' : req.type === 'dogum_gunu' ? 'bg-purple-500' : req.type === 'organizasyon' ? 'bg-emerald-500' : 'bg-orange-500'}`}>{typeLabels[req.type] || 'REZERVASYON'}</div>
  ...
  // =======================================================================
  // ZİYARETÇİ İÇİN SABİT YARDIMCILAR (MODALLAR, NAV, FOOTER)
  // =======================================================================
  const renderNavbar = () => (
    <div className="fixed top-0 left-0 w-full z-50 flex justify-center pointer-events-none">
      <nav className={`w-full pointer-events-auto transition-all duration-500 ${isScrolled ? 'bg-white/95 backdrop-blur-md shadow-md py-3 border-b border-slate-200' : 'bg-transparent py-5 sm:py-6'}`}>
        <div className="w-full px-4 sm:px-8 lg:px-12 xl:px-24 flex items-center justify-between">
          <div className={`transition-all duration-500 cursor-pointer flex items-center justify-center bg-transparent shrink-0 ${isScrolled ? 'h-10 sm:h-12' : 'h-12 sm:h-16'}`} onClick={() => { setShowMenuPage(false); window.scrollTo(0,0); }}>
            <img src="/salaaslogobg.png" alt="Salaaş Logo" className="h-full w-auto object-contain bg-transparent" />
          </div>
          <div className={`hidden lg:flex flex-1 justify-center items-center gap-6 xl:gap-12 font-bold text-sm xl:text-base transition-colors duration-500 ${isScrolled || showMenuPage ? 'text-slate-700' : 'text-white drop-shadow-md'}`}>
            <button onClick={() => {setShowMenuPage(false); setTimeout(()=>handleScrollToId('hakkimizda'),100);}} className="hover:text-orange-500 transition-colors whitespace-nowrap">Biz Kimiz?</button>
            <button onClick={() => {setShowMenuPage(false); setTimeout(()=>handleScrollToId('lezzetler'),100);}} className="hover:text-orange-500 transition-colors whitespace-nowrap">Lezzetler</button>
          ...
          <div className="shrink-0 flex items-center justify-end gap-3 sm:gap-4">
            <button onClick={() => setShowRequestModal(true)} className={`hidden sm:flex items-center gap-2 px-4 py-2.5 xl:px-6 xl:py-3 rounded-full text-xs xl:text-sm font-bold tracking-widest uppercase transition-all shadow-md ${isScrolled || showMenuPage ? 'bg-white text-emerald-700 border border-emerald-200 hover:bg-emerald-50' : 'bg-emerald-700/80 backdrop-blur-sm text-white hover:bg-emerald-600 border border-emerald-500/50'}`}>
              <CalendarDays size={16} /> Rezervasyon
            </button>
            <button onClick={() => { setShowMenuPage(true); window.history.pushState({}, '', '/menu'); window.scrollTo(0,0); }} className="shine-effect bg-black/80 text-[#FBE18D] border border-[#FBE18D]/30 px-5 py-2.5 sm:px-6 sm:py-3 xl:px-8 xl:py-3.5 rounded-full text-xs sm:text-sm font-black tracking-widest uppercase hover:bg-black hover:border-[#FBE18D] hover:scale-105 transition-all shadow-lg whitespace-nowrap flex items-center gap-2">
      ...
      <div className="w-full mx-auto px-6 sm:px-12 lg:px-24 grid grid-cols-1 md:grid-cols-3 gap-16 lg:gap-24">
        ...
        <div className="flex flex-col items-center md:items-start text-center md:text-left">
          ...
          <ul className="space-y-5 text-base lg:text-lg font-medium mb-10 text-slate-300">
            <li><button onClick={() => { setShowMenuPage(true); window.history.pushState({}, '', '/menu'); window.scrollTo(0,0); }} className="hover:text-white transition-colors flex items-center justify-center md:justify-start gap-3"><ChevronRight size={18} className="text-orange-500"/> Dijital Menü</button></li>
          </ul>
              ...
              {requestSuccess ? (
                ...
                <form onSubmit={submitRequest} className="p-8 sm:p-10 space-y-6">
                  ...
                  <div>
                    ...
                    <textarea name="notes" value={requestData.notes} onChange={handleRequestChange} rows={3} required={(requestData.type === 'dogum_gunu' || requestData.type === 'organizasyon')} className="w-full bg-white px-5 py-4 rounded-2xl border-2 focus:ring-4 outline-none font-medium text-slate-800 transition-all resize-none text-base border-slate-200 focus:ring-orange-500/10 focus:border-orange-500" placeholder="Özel isteklerinizi yazın."></textarea>
                  </div>
                  <button type="submit" className="w-full text-white font-black tracking-widest uppercase py-5 rounded-2xl transition-all shadow-lg hover:shadow-xl mt-4 flex items-center justify-center gap-3 hover:-translate-y-1 text-lg bg-orange-500 hover:bg-orange-600">Talebi Gönder <ArrowRight size={24} /></button>
                </form>
              )}
            </div>
          </div>
              ...
              <section id="lezzetler" className="w-full py-20 md:py-32 border-y border-slate-200/50 bg-slate-50">
                <div className="w-full mx-auto px-4 sm:px-8 lg:px-12 xl:px-24">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-12 gap-6">
                    ...
                    <button onClick={() => { setShowMenuPage(true); window.history.pushState({}, '', '/menu'); window.scrollTo(0,0); }} className="flex items-center gap-2 text-sm lg:text-base font-bold text-slate-500 hover:text-orange-600 transition-all uppercase tracking-widest group">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 xl:gap-10">
                    <div onClick={() => { setShowMenuPage(true); window.history.pushState({}, '', '/menu'); window.scrollTo(0,0); }} className="lg:col-span-2 sm:col-span-2 rounded-[2rem] lg:rounded-[3rem] overflow-hidden shadow-lg hover:shadow-2xl hover:shadow-orange-500/20 hover:-translate-y-2 transition-all duration-500 relative group h-80 sm:h-96 md:h-[450px] cursor-pointer border border-slate-100 bg-white">
                      <img src={encodeURI("/salaskoy.jpg")} alt="Salaaş Köy Kahvaltısı" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                    ...
                    <div onClick={() => { setShowMenuPage(true); window.history.pushState({}, '', '/menu'); window.scrollTo(0,0); }} className="rounded-[2rem] lg:rounded-[3rem] overflow-hidden shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 relative group h-80 sm:h-96 md:h-[450px] cursor-pointer border border-slate-100 bg-white">
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-slate-900/90 to-transparent flex flex-col justify-end p-8 lg:p-10">
                         ...
                         <p className="text-slate-300 text-sm lg:text-base mt-2 font-medium">Şefin tavsiyesi, özel soslu.</p>
                      </div>
                    </div>
                    <div onClick={() => { setShowMenuPage(true); window.history.pushState({}, '', '/menu'); window.scrollTo(0,0); }} className="rounded-[2rem] lg:rounded-[3rem] overflow-hidden shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 relative group h-80 sm:h-96 md:h-[450px] cursor-pointer border border-slate-100 bg-white">
                    ...
                    <div onClick={() => { setShowMenuPage(true); window.history.pushState({}, '', '/menu'); window.scrollTo(0,0); }} className="rounded-[2rem] lg:rounded-[3rem] overflow-hidden shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 relative group h-80 md:h-[450px] cursor-pointer border border-slate-100 bg-white">
  ...
  return (
    <div className={`min-h-screen font-sans text-slate-800 pb-12 print:bg-white print:pb-0 relative transition-colors duration-500 w-full overflow-x-hidden ${activePage === 'restoran' ? 'bg-slate-50' : activePage === 'mac' ? 'bg-[#f0f4f8]' : 'bg-slate-100'}`}>
      ...
      {activePage === 'restoran' && <div className="fixed inset-0 z-0 pointer-events-none opacity-[0.03] print:hidden w-full h-full" style={{ backgroundImage: 'radial-gradient(#f97316 1px, transparent 1px)', backgroundSize: '40px 40px' }}></div>}
      {activePage === 'mac' && <div className="fixed top-20 right-10 z-0 pointer-events-none opacity-[0.03] text-blue-900 rotate-12 print:hidden"><MonitorPlay size={400} strokeWidth={1} /></div>}
